...
export const items = [
  {
    id: 1,
    url: "/award1.png",
    title: "63rd National Film Award",
    description:
      "A breathtaking view of misty mountains shrouded in clouds, creating an ethereal landscape.",
    tags: ["Misty", "Mountains", "Clouds", "Ethereal", "Landscape"],
  },
  ...
  },
  {
    id: 4,
    url: "/award1.png",
    title: "National Geographic Short Film",
    description:
      "A calm mountain lake perfectly reflecting the surrounding peaks and sky, creating a mirror-like surface.",
    tags: ["Lake", "Reflection", "Mountains", "Tranquil", "Mirror"],
  },
...
function Gallery({ items, setIndex, index }) {
  return (
    <div className="w-fit mx-auto gap-4 flex-row flex max-lg:flex-col pb-20 pt-10">
      {items.slice(0, 5).map((item, i) => (
        <motion.div
          whileTap={{ scale: 0.95 }}
          className={`rounded-xl relative
          ...
          <AnimatePresence mode="wait">
            {index === i && (
              ...
              >
                ...
                <motion.p
                  variants={article}
                  className="leading-[120%] text-white"
                >
                  {item?.description}
                </motion.p>
              </motion.article>
